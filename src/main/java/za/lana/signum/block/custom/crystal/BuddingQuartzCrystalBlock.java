...
public class BuddingQuartzCrystalBlock
        extends QuartzCrystalBlock {
    public static final int GROW_CHANCE = 5;
    public static final int DURATION_EFFECT = 100;
    public static final int DISPLAY_CHANCE = 16;


    public BuddingQuartzCrystalBlock(Settings settings) {
        super(settings);
    }
    ...
    public void randomTick(BlockState state, ServerWorld world, BlockPos pos, Random random) {
        ...
        if (BuddingQuartzCrystalBlock.canGrowIn(blockState)) {
            block = ModBlocks.SMALL_QUARTZ_CRYSTAL_BUD;
        } else if (blockState.isOf(ModBlocks.SMALL_QUARTZ_CRYSTAL_BUD) && blockState.get(FireCrystalClusterBlock.FACING) == direction) {
            block = ModBlocks.MEDIUM_QUARTZ_CRYSTAL_BUD;
        } else if (blockState.isOf(ModBlocks.MEDIUM_QUARTZ_CRYSTAL_BUD) && blockState.get(FireCrystalClusterBlock.FACING) == direction) {
            block = ModBlocks.LARGE_QUARTZ_CRYSTAL_BUD;
        } else if (blockState.isOf(ModBlocks.LARGE_QUARTZ_CRYSTAL_BUD) && blockState.get(FireCrystalClusterBlock.FACING) == direction) {
            block = ModBlocks.QUARTZ_CRYSTAL_CLUSTER;
        }
        if (block != null) {
