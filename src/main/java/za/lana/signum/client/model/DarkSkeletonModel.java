...
public class DarkSkeletonModel<T extends DarkSkeletonEntity>
    ...
    private final ModelPart darkskeleton;
    ...
    public DarkSkeletonModel(ModelPart root) {
        ...
        this.leftArm = darkskeleton.getChild("body").getChild("leftArm");
    ...
    @Override
    public void setAngles(DarkSkeletonEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);
        this.rightArm.pitch = MathHelper.cos(limbSwing * 0.6662f + (float)Math.PI) * 2.0f * limbSwingAmount * 0.5f;
        this.rightArm.yaw = 0.0f;
        this.rightArm.roll = 0.0f;
        this.animateMovement(DarkSkeletonAnimations.DARKSKELETON_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAniState, DarkSkeletonAnimations.DARKSKELETON_IDLE, ageInTicks, 1f);
        this.updateAnimation(entity.attackAniState, DarkSkeletonAnimations.DARKSKELETON_ATTACK, ageInTicks, 1f);
    }
