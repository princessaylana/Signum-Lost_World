...
import za.lana.signum.client.animation.EnderSkeletonAnimations;
...
public class EnderSkeletonModel<T extends EnderSkeletonEntity>
        extends SinglePartEntityModel<T> implements ModelWithArms {
    private final ModelPart enderskeleton;
    ...
    public EnderSkeletonModel(ModelPart root) {
        ...
        this.leftArm = enderskeleton.getChild("body").getChild("leftArm");
    ...
    @Override
    public void setAngles(EnderSkeletonEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);
        this.rightArm.pitch = MathHelper.cos(limbSwing * 0.6662f + (float)Math.PI) * 2.0f * limbSwingAmount * 0.5f;
        this.rightArm.yaw = 0.0f;
        this.rightArm.roll = 0.0f;
        this.animateMovement(EnderSkeletonAnimations.ENDERSKELETON_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAniState, EnderSkeletonAnimations.ENDERSKELETON_IDLE, ageInTicks, 1f);
        this.updateAnimation(entity.attackAniState, EnderSkeletonAnimations.ENDERSKELETON_ATTACK, ageInTicks, 1f);
    }
