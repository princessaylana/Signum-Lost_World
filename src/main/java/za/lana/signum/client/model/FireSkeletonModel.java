...
import za.lana.signum.client.animation.FireSkeletonAnimations;
import za.lana.signum.entity.hostile.FireSkeletonEntity;

public class FireSkeletonModel<T extends FireSkeletonEntity>
        extends SinglePartEntityModel<T> implements ModelWithArms {
    private final ModelPart fireskeleton;
    ...
    public FireSkeletonModel(ModelPart root) {
        ...
        this.leftArm = fireskeleton.getChild("body").getChild("leftArm");
    ...
    @Override
    public void setAngles(FireSkeletonEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);
        this.rightArm.pitch = MathHelper.cos(limbSwing * 0.6662f + (float)Math.PI) * 2.0f * limbSwingAmount * 0.5f;
        this.rightArm.yaw = 0.0f;
        this.rightArm.roll = 0.0f;
        this.animateMovement(FireSkeletonAnimations.FIRESKELETON_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAniState, FireSkeletonAnimations.FIRESKELETON_IDLE, ageInTicks, 1f);
        this.updateAnimation(entity.attackAniState, FireSkeletonAnimations.FIRESKELETON_ATTACK, ageInTicks, 1f);
    }
