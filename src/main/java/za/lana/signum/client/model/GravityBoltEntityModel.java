...
public class GravityBoltEntityModel extends SinglePartEntityModel<GravityBoltEntity> {

	private final ModelPart bolt;
	public GravityBoltEntityModel(ModelPart root) {
	...
	@Override
	public void setAngles(GravityBoltEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
		this.getPart().traverse().forEach(ModelPart::resetTransform);
		this.animateMovement(BoltAnimations.BOLT_MOVE, limbSwing, limbSwingAmount, 2f, 2.5f);
	}
	@Override
	public void render(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha) {
		bolt.render(matrices, vertexConsumer, light, overlay, red, green, blue, 0.5F);
	}
