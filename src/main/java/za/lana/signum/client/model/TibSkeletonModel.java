...
public class TibSkeletonModel<T extends TibSkeletonEntity>
        extends SinglePartEntityModel<T> implements ModelWithArms {
    private final ModelPart tibskeleton;
    private final ModelPart head;
    private final ModelPart rightArm;
    private final ModelPart leftArm;

    public TibSkeletonModel(ModelPart root) {
        this.tibskeleton = root.getChild("mainBody");
        this.head = tibskeleton.getChild("body").getChild("head");
        this.rightArm = tibskeleton.getChild("body").getChild("rightArm");
        this.leftArm = tibskeleton.getChild("body").getChild("leftArm");
    ...
    @Override
    public void setAngles(TibSkeletonEntity entity, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch) {
        this.getPart().traverse().forEach(ModelPart::resetTransform);
        this.setHeadAngles(netHeadYaw, headPitch);
        this.rightArm.pitch = MathHelper.cos(limbSwing * 0.6662f + (float)Math.PI) * 2.0f * limbSwingAmount * 0.5f;
        this.rightArm.yaw = 0.0f;
        this.rightArm.roll = 0.0f;
        this.animateMovement(TibSkeletonAnimations.TIBSKELETON_WALK, limbSwing, limbSwingAmount, 2f, 2.5f);
        this.updateAnimation(entity.idleAniState, TibSkeletonAnimations.TIBSKELETON_IDLE, ageInTicks, 1f);
        this.updateAnimation(entity.attackAniState, TibSkeletonAnimations.TIBSKELETON_ATTACK, ageInTicks, 1f);
    }
    ...
    public void render(MatrixStack matrices, VertexConsumer vertexConsumer, int light, int overlay, float red, float green, float blue, float alpha) {
        tibskeleton.render(matrices, vertexConsumer, light, overlay, red, green, blue, alpha);
    }
