...
public class AirShipFindHomeGoal
    ...
    private boolean tryFindHome() {
        ...
        Optional<BlockPos> optional = serverWorld.getPointOfInterestStorage().getPosition(registryEntry ->
                ...
                PointOfInterestStorage.OccupationStatus.ANY, blockPos, 256, Random);
        if (optional.isEmpty()) {
            return false;
        }
    ...
    public void tick() {
        if (this.mob.getNavigation().isIdle()) {
            ...
            Vec3d vec3d2 = NoPenaltyTargeting.findTo(this.mob, 16, 16, vec3d, 0.3141592741012573);
            if (vec3d2 == null) {
                vec3d2 = NoPenaltyTargeting.findTo(this.mob, 8, 8, vec3d, 1.5707963705062866);
            }
            if (vec3d2 == null) {
                this.finished = true;
                return;
