    ...
    public static void init() {
        ServerPlayNetworking.registerGlobalReceiver(ABKEY_INPUT_PACKET, (server, player, handler, buffer, sender) -> {
            ...
            if (entity instanceof AirBalloonEntity airBalloon) {
                airBalloon.isflyUpKeyPressed = airBalloon.isFlyUpKeyPressed();
                airBalloon.isflyDownKeyPressed = airBalloon.isFlyDownKeyPressed();
                airBalloon.updateInputs(airBalloon.isFlyUpKeyPressed(), airBalloon.isFlyDownKeyPressed());
            }
        });
    }
}
